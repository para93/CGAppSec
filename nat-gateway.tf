...
resource "aws_eip" "eip-for-appsec-nat-gateway-1" {
  vpc    = true
...
resource "aws_nat_gateway" "appsec-nat-gateway-1" {
  allocation_id = aws_eip.eip-for-appsec-nat-gateway-1.id
  subnet_id     = aws_subnet.public-subnet-1.id

  tags   = {
    Name = "AppSec NAT GW Public Subnet 1"
  }
}
...
resource "aws_nat_gateway" "nat-gateway-2" {
  ...
  tags   = {
    Name = "AppSec NAT GW Public Subnet 2"
  }
}
...
resource "aws_route_table" "private-route-table-1" {
  ...
  route {
    ...
    nat_gateway_id  = aws_nat_gateway.appsec-nat-gateway-1.id
  }

  tags   = {
    Name = "AppSec Private Route Table 1"
  }
}
...
resource "aws_route_table" "private-route-table-2" {
  ...
  tags   = {
    Name = "AppSec Private Route Table 2"
  }
}
